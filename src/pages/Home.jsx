...
export default function Home() {
  ...
  return (
    <div className="px-4 pb-8 pt-6 md:px-8">
      <h1 className="text-lg font-medium text-gray-900">잇벤토리</h1>
      <p className="mt-1 text-sm text-gray-500">있는 재료로 오늘 뭐 만들지 찾아보세요.</p>
      ...
      {ingredients.length === 0 ? (
        ...
        <>
          <button
            ...
            className="mt-5 w-full rounded-xl bg-white p-4 text-left shadow-sm ring-1 ring-gray-100"
          >
            <p className="text-sm text-gray-500">보유 재료</p>
            <p className="mt-1 text-xl font-medium text-gray-900">
              {ingredients.length}개
              {expiringSoon > 0 && (
                <span className="ml-2 text-sm font-normal text-amber-600">유통기한 임박 {expiringSoon}개</span>
              )}
            </p>
            <p className="mt-1 text-xs text-emerald-600">냉장고 보기 →</p>
          </button>

          <div className="mt-6 flex items-center justify-between">
            <h2 className="text-sm font-medium text-gray-900">오늘 만들 수 있는 요리</h2>
            <button onClick={() => navigate('/recipes')} className="flex items-center gap-1 text-xs text-emerald-600">
