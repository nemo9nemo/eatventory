...
test.describe('보관 구분 자동 추천', () => {
  ...
  test('실온 재료명을 입력하면 실온이 자동 선택된다', async ({ page }) => {
    ...
    await expect(page.getByTestId('category-room')).toHaveClass(/bg-curry-600/)
  })

  test('냉동 재료명을 입력하면 냉동이 자동 선택된다', async ({ page }) => {
    await page.getByPlaceholder('예: 계란').fill('새우')
    await expect(page.getByTestId('category-frozen')).toHaveClass(/bg-curry-600/)
  })
  ...
  test('매칭되는 재료명이 없으면 자동 추천 문구가 뜨지 않고 기본값(냉장)을 유지한다', async ({ page }) => {
    ...
    await expect(page.getByTestId('category-fridge')).toHaveClass(/bg-curry-600/)
  })
